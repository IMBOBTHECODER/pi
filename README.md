...
You need a C++ compiler with OpenMP (**g++** or **clang++**) and **GMP**. On
Windows, build under WSL.

**Compiler**

```sh
sudo apt update && sudo apt install g++ make m4          # Debian / Ubuntu / WSL
sudo dnf install gcc-c++ make m4                         # Fedora / RHEL
sudo pacman -S gcc make m4                               # Arch
brew install gcc gmp   # macOS: Apple Clang has no OpenMP, build with g++-14
```

**GMP — build it yourself.** Distro `libgmp-dev` is compiled for a generic
baseline CPU and ships generic FFT thresholds. Nearly all of this program's time
is inside GMP, so a tuned local build is worth more than any compiler flag here.

```sh
curl -O https://gmplib.org/download/gmp/gmp-6.3.0.tar.xz
tar xf gmp-6.3.0.tar.xz && cd gmp-6.3.0
./configure --enable-cxx --enable-fat=no --build=native
make -j"$(nproc)"
make check                  # do not skip this — GMP is the whole computation
sudo make install           # installs to /usr/local
sudo ldconfig
```

`--build=native` lets configure pick the CPU-specific assembly path instead of a
generic one. Optionally follow with `make tune` in `tune/` and rebuild with the
thresholds it prints; that is where the FFT-crossover gains come from.

The install lands in `/usr/local/{include,lib}`, which is why the build line
below passes `-I/usr/local/include -L/usr/local/lib` and an rpath — without the
rpath the linker finds your build but the loader still picks up the distro one at
runtime. If you would rather use the packaged GMP, install `libgmp-dev` (or
`gmp-devel` / `gmp`) and drop those three flags.

Check it worked:

```sh
g++ --version
echo '#include <gmp.h>' | g++ -I/usr/local/include -x c++ -E - >/dev/null
```
...
g++ -O3 -march=native -DNDEBUG chudnovsky.cpp \
    ...
    -lgmp -fopenmp -o main
ldd main | grep gmp        # must show /usr/local/lib, not /usr/lib/x86_64-linux-gnu
```
...
- The `-I` / `-L` / `-rpath` trio points at the locally built GMP. **Check `ldd`
  every time** — if the loader silently falls back to the distro library your
  benchmark numbers are measuring the wrong thing.
- On a packaged GMP, drop those three flags and just link `-lgmp`.

## Run

```
./main                     # prompts for a digit count
./main 50M                 # 50M digits; k/M/G suffixes and 1e8 also work
./main 1M -o               # also write the digits to pi.txt
./main 1M -o digits.txt    # ...or to a name you choose
./main 50M -t 8            # cap OpenMP at 8 threads
./main -h                  # full option list
```

`-p` / `--print` are aliases for `-o` / `--out`. Options take their value joined
or separated: `-t 8` and `-t=8` both work, `-t8` does not.
...
- `build.sh` — the build line above, including the local-GMP paths.
