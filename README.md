...
Computes π to a requested number of digits with the Chudnovsky algorithm and
GMP. The fast path (`bs_chudnovsky`) uses **binary splitting**, parallelised with
OpenMP, plus a parallel base-10 conversion for output.
...
- `chudnovsky.cpp` — the C++/GMP implementation (designs: incremental + binary
  splitting).
